...
//  * @Date: 2018-04-16 10:10:39 
//  * @Last Modified by:   Atilla Tanrikulu 
//  * @Last Modified time: 2018-04-16 10:10:39 
//  */
...
//         public BaseRepositorySQL(DatabaseContext databaseContext, ServiceContext serviceContext)
//         {
...
//             long result = DatabaseContext.Connection.Execute(sql, paramList);

//             if (result == 1)
//             {
//                 result = DatabaseContext.Connection.ExecuteScalar<long>("SELECT last_insert_rowid()");
//             }
...
//             TEntity obj;
//             if (!IsInterface(type)) return obj = DatabaseContext.Connection.Query<TEntity>(sql, dynParms).FirstOrDefault();

//             //TODO:Atilla => Asagisi daha sonra calisir hale getirilecek
//             var res = DatabaseContext.Connection.Query(sql, dynParms).FirstOrDefault() as IDictionary<string, object>;
...
//             LogSql(sql, paramList); //TODO:Atilla => Daha sonra iptal edilecek
//             return DatabaseContext.Connection.Execute(sql, paramList);
//         }
...
//         {
//             if (ServiceContext.UserInfo == null || ServiceContext.UserInfo.Username == null) throw new Exception("Islem yapan kullanici bilgisi bulunamadi!");
//             int userId = 0;
//             if (!int.TryParse(ServiceContext.UserInfo.Username, out userId)) throw new Exception("Islem yapan kullanici bilgisi, Int degere donusturulemiyor!");

//             return DatabaseContext.Connection.Execute($@"UPDATE {GetTableName(typeof(TEntity))}
...
//                 //          {sqlSelect} {sqlWhere};";
//                 //using (var multi = DatabaseContext.Connection.QueryMultiple(sql, param: paramList))
//                 //{
...
//             return DatabaseContext.Connection.Query<TEntity>($"SELECT * FROM {tableName} T1 {sqlWhere} {OrderBy(pagingDto, "T1")} {PagingSql(pagingDto)}", paramList).ToList();
//         }
...
//         {
//             return DatabaseContext.Connection.Query<TEntity>($@"SELECT * FROM {GetTableName(typeof(TEntity))} WHERE ISACTIVE=1 AND ID IN @ID ", new { Id = id });
//         }
...
//             return DatabaseContext.Connection.Execute($@"UPDATE {GetTableName(typeof(TEntity))} SET ISACTIVE=0, UPDATEDATE=SYSDATE, UPDATEDBY=@UpdatedBy WHERE Id IN @ID", new { Id = id, UpdatedBy = userId });
//         }
...
//         {
//             return DatabaseContext.Connection.Execute($@"DELETE FROM {GetTableName(typeof(TEntity))} WHERE Id=@ID", new { Id = id });
//         }
...
//         public virtual int DeleteHard(int[] id)
//         {
//             return DatabaseContext.Connection.Execute($@"DELETE FROM {GetTableName(typeof(TEntity))} WHERE Id IN =@ID", new { Id = id });
//         }
...
//             if (!IsInterface(type)) return DatabaseContext.Connection.Query<TEntity>(sql);
//             //TODO:Atilla => Asagisi daha sonra calisir hale getirilecek
//             var result = DatabaseContext.Connection.Query(sql);
...
//         {
//             return DatabaseContext.Connection.QuerySingleOrDefault($"SELECT count(Id) FROM {GetTableName(typeof(TEntity))} WHERE ISACTIVE=1 ");
//         }

//         //public virtual int Count(Predicate where)
//         //{
//         //    return DatabaseContext.Connection.QuerySingleOrDefault($"SELECT count(Id) FROM {GetTableName(typeof(TEntity))} where {where}");
//         //}
...
//             return DatabaseContext.Connection.Query<int>($"{sqlSelect} {sqlWhere}", paramList).SingleOrDefault();
//         }
